...
writeDeviceKeyValues = function(keyValue) {
    for (index in keyValue) {
        deviceProperties[index] = keyValue[index];
    }
};
...
(function() {
	...
	viewporter = {
		...
		ACTIVE: (function() {
            ...
            if('webkitAudioContext' in window) writeDeviceKeyValues({Audio:"webkitAudioContext"});
            if('AudioContext' in window) writeDeviceKeyValues({Audio:"AudioContext"});
        //    if('AudioContext' in window) writeDeviceKeyValues({Audio:"AudioContext"});

			// iPad's don't allow you to scroll away the UI of the browser
			if((/ipad/i).test(navigator.userAgent)) {
                writeDeviceKeyValues({DeviceType:"iPad", TouchInput:true})
				return false;
			}

            // touch enabled devices
            if('ontouchstart' in window) {
                writeDeviceKeyValues({DeviceType:"TouchDevice", TouchInput:true})
                return true;
            }

			// WebOS has no touch events, but definitely the need for viewport normalization
			if((/webos/i).test(navigator.userAgent)) {
                writeDeviceKeyValues({DeviceType:"webos", TouchInput:false})
				return true;
			}
